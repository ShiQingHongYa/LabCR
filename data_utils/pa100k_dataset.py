...
# https://github.com/valencebond/Rethinking_of_PAR/blob/0baa462607/dataset/pedes_attr/pedes.py
class PedesAttr(data.Dataset):

    def __init__(self, cfg, split, transform=None, target_transform=None, idx=None):

        if cfg.dataname=='PA100K':
            data_path = '/data/sqhy_data/PA100K/dataset_all.pkl'
        elif cfg.dataname=='RAP':
            data_path = '/data/sqhy_data/RAP/dataset_all.pkl'

        print("which pickle", data_path)

        dataset_info = pickle.load(open(data_path, 'rb+'))
        if cfg.dataname == 'PA100K':
            dataset_info.root = '/data/sqhy_data/PA100K/release_data'
        elif cfg.dataname == 'RAP':
            dataset_info.root = '/data/sqhy_data/RAP/'
        else:
            dataset_info.root = '/data/sqhy_data/PA100K/release_data'
        ...
        self.attr_id = dataset_info.attr_name  # 属性名称
        self.attr_num = len(self.attr_id) # 属性数量
