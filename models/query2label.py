...
class Qeruy2Label(nn.Module):
    ...
    def forward(self, input):
        ...
        src, pos = src[-1], pos[-1]   # src[128,2048,7,7]

        query_input = self.query_embed.weight
        hs = self.transformer(self.input_proj(src), query_input, pos)[0] # B,K,d (1,2*B/gpu,28,2048)
        features = hs[-1] # (2*B/gpu,28,2048)
        out = self.fc(hs[-1])  # (2*B/gpu,28)
